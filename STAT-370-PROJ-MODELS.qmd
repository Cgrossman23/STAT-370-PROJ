...
neighbors_a <- dnearneigh(coords_a, d1 = 0, d2 = od)

plot(neighbors_a,coords_a, col = "red")

site_a <- nb2listw(neighbors_a, zero.policy = TRUE)
...
neighbors_c <- dnearneigh(coords_c, d1 = 0, d2 = od)

plot(neighbors_c,coords_c, col = "red")

site_c <- nb2listw(neighbors_c, zero.policy = TRUE)
```
...
point_colors <- ifelse(wave_d$presence_absence == 1, "red", "blue")

plot(neighbors_d, wave_d$coords_d, col = "red")

points(wave_d$coords_d, col = point_colors, pch = 16)
...
sqrt_f=sqrt(density)~cost_small+fetch_projected+water_depth+slope_use
cbrt_f <- I(density^(1/3)) ~ cost_small + fetch_projected + water_depth + slope_use

summary(wave_d$density)
...
plot(resid(model_a) ~ fitted(model_a), main = "Residuals vs Fitted", xlab = "Fitted Values", ylab = "Residuals")
...
plot(resid(model_b) ~ fitted(model_b), main = "Residuals vs Fitted", xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red", lty = 2)
...
plot(resid(model_c) ~ fitted(model_c), main = "Residuals vs Fitted", xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red", lty = 2)
...
plot(resid(model_d) ~ fitted(model_d), main = "Residuals vs Fitted", xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red", lty = 2)

qqnorm(resid(model_d))
qqline(resid(model_d), col = "red", lty = 2)

lm.morantest(model_a , site_a)
lm.morantest(model_b , site_b)
lm.morantest(model_c , site_c)
lm.morantest(model_d , site_d)

lm.RStests(model_a , site_a, test = "all")
lm.RStests(model_b , site_b, test = "all")
lm.RStests(model_c , site_c, test = "all")
lm.RStests(model_d , site_d, test = "all")
...
```{r}
model_zero_d <- zeroinfl(f , data = wave_d, dist="negbin")
