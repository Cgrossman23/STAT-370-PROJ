...
```{r}
model02 <- glmmTMB(density ~ exposure*water_depth 
                            + (1 | site_individual),
                    ziformula = ~ 1, 
                    family = "nbinom2",
                    data = wave4)
...
spatial <- glmmTMB(
  density ~ water_depth + exposure + spatial_lag1 + spatial_lag2 + spatial_lag3 + (1 | site_individual),
  ziformula = ~ 1, 
  family = "nbinom2",
  data = wave4
)
