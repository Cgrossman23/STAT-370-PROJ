...
#### **II.c.1.1. Bayesian Analysis of Additional Data**

Further data on plant growth along and in the St. Mary's River obtained by researchers from the University of Michigan Biological Station was provided for the sake of validating results of the initial dataset, along with fixing any issues related to the extreme presence of zeroes among the Exposed group. This new data contained 443 observations of Bullrush and Cattail growth across the entirety of the St. Mary's Channel, and included key variables like water depth, slope, and growth density. While Exposure was not measured by the researchers at the time of data collection, a GIS buffer was used to post-hoc determine Exposure based upon GPS coordinates. Sixty of sites surveyed were both exposed to the channel and had either Bullrush or Cattail presence, meaning that we may not have to employ a zero-inflated model. This data was also not collected in transects, and was instead collected in clusters, with some sites even located hundreds of meters away from the nearest site. This meant that a uniformly applied spatial correlation algorithm similar to the one used across transects as in the previous sections may not be appropriate.

Bayesian analysis using the rstanarm package was performed, with a weakly informative prior chosen by the package's built-in algorithm. Based off of the results from the mixed-effects model utilizing a Negative Binomial distribution, a Negative Binomial was also used as the likelihood function for Bayesian analysis with water depth and exposure used as predictors.
...
### **III.g. Results of Bayesian Model**

Results from the Negative Binomial Bayesian model backed the findings of our initial findings from the initial, smaller dataset. Initial results from the Negative Binomial generated a very small credible interval for the water depth variable. After inspection of the very small values from the sampled Posterior, the water depth variable was scaled by divided each observation by its standard deviation to adjust for its large size when compared to the total density outcome variable, which may have been shrinking its parameter estimate. MCMC sampling converged quickly with the scaled water depth variable and yielded the following Posterior Estimates and Credible Intervals:

```         
Estimates:
                        mean   sd   10%   50%   90%
(Intercept)            1.9    0.2  1.6   1.9   2.2 
water_depth            0.2    0.2  0.0   0.2   0.4 
Exposure_dummy        -3.7    0.4 -4.2  -3.7  -3.2 
reciprocal_dispersion  0.1    0.0  0.1   0.1   0.2 
```

![](000024.png){width="516"}

Within the framework of Bayesian Statistics we can say that there is a 95% chance that the true effect of exposure to channel traffic in the St. Mary's river is significantly different from zero. In this model we cannot conclusively say that water depth has any effect on density of Cattail and Bullrush. Additionally we see the credible interval for the reciprocal dispersion parameter within the Negative Binomial distribution indicates that the wide geographic spread of site locations has contributed to added variance, and that the Negative Binomial distribution is right for the task. Below is a diagnostic post posterior histogram for determining the fit of our model which compares simulated draws from the posterior with the actual observations from the dataset. The top left histogram is made from the observations and follows a similar shape to our simulated histograms, meaning that our assumption of a Negative Binomial distribution is valid.

![](000033.png?resize=11)

#  IV. Conclusion

The main goal of the project and the client was to find a suitable, accurate, and interpretable model for the St. Mary's River. With understanding the distributions, creating spatial lags to account for correlation, and the inclusions of mixed effects, the best model was the site-specific logistic regression. The binary response variable was deemed to be the most appropriate, due to the large number of zeros. Analysis of additional data using Bayesian methods yielded similar results to the site-specific logistic regression, and confirmed our findings that channel exposure does have a statistically significant effect on the growth of Bullrush and Cattail along the St. Mary's river.
...
```{r}
new_data <- read.csv(curl("https://raw.githubusercontent.com/Cgrossman23/STAT-370-PROJ/refs/heads/main/GLRI_data_added.csv"))
nd_clean <- new_data |> mutate(total_density = TYP_D + SCH_D)
```

```{r}
nd_clean$total_density<- as.integer(nd_clean$total_density)
nd_clean$Exposure_dummy <- factor(nd_clean$Exposure, levels = c("Unexposed", "Exposed"))
nd_clean <- nd_clean[complete.cases(nd_clean$water_depth, nd_clean$Exposure_dummy),]
nd_clean$Exposure_dummy <- ifelse(nd_clean$Exposure == "Exposed", 1, 0)
nd_scale1 <- nd_clean
nd_scale1$water_depth<- scale(nd_scale1$water_depth)

fit1 <- stan_glm(total_density~water_depth + Exposure_dummy, data = nd_scale1, seed = 12, family = neg_binomial_2(link = "log"), chains = 4, iter = 2000, warmup = 1000)
summary(fit1)
print(fit1)
pp_check(fit1, nreps = 100)
```

```{r}
posteriorSamples <- as.matrix(fit1)
param_names <- colnames(posteriorSamples)
param_names<- param_names[!grepl("^(sigma|mean_PPD|log_posterior)",param_names)]
mcmc_areas(posteriorSamples, pars = param_names[1:min(5,length(param_names))], probs = .95)+labs(title = "95% Credible Intervals ")
y_rep <- posterior_predict(fit1)
ppc_dens_overlay(y = fit1$y,yrep = y_rep[1:50,], )+labs(title = "Posterior Predictive Check", x = "y",y = "Density")
y_fitted<- colMeans(posterior_predict(fit1))
residuals <- fit1$y - y_fitted
```

```{r}
color_scheme_set("blue")
ppc_hist(y= fit1$y, yrep = posterior_predict(fit1, draws = 8), bins = 30)+xlim(0,20)+ylim(0,20)+labs(title = "Posterior Predictive Check Histogram")
ppc_boxplot(y = fit1$y, yrep = posterior_predict(fit1,draws =8))+labs(title = "Posterior Predictive Check Boxplot")
```
